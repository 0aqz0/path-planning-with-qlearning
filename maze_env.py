...
class Maze(tk.Tk, object):
    ...
    def build_maze(self):
        ...
        # 炸弹 1
        bomb1_center = origin + np.array([40 * random.randint(1, WIDTH - 1), 40 * random.randint(1, HEIGHT - 1)])
        ...
        # 炸弹 2
        random.seed(a=1)
        bomb2_center = origin + np.array([40 * random.randint(1, WIDTH - 1), 40 * random.randint(1, HEIGHT - 1)])
        ...
        # 炸弹 3
        random.seed(a=3)
        bomb3_center = origin + np.array([40 * random.randint(1, WIDTH - 1), 40 * random.randint(1, HEIGHT - 1)])
        self.bomb3 = self.canvas.create_rectangle(
            bomb3_center[0] - 15, bomb2_center[1] - 15,
            bomb3_center[0] + 15, bomb2_center[1] + 15,
            fill='red')

        # 炸弹 4
        random.seed(a=6)
        bomb4_center = origin + np.array([40 * random.randint(1, WIDTH - 1), 40 * random.randint(1, HEIGHT - 1)])
        self.bomb4 = self.canvas.create_rectangle(
            bomb4_center[0] - 15, bomb2_center[1] - 15,
            bomb4_center[0] + 15, bomb2_center[1] + 15,
            fill='red')

        # 炸弹 5
        random.seed(a=5)
        bomb5_center = origin + np.array([40 * random.randint(1, WIDTH - 1), 40 * random.randint(1, HEIGHT - 1)])
        self.bomb5 = self.canvas.create_rectangle(
            bomb5_center[0] - 15, bomb2_center[1] - 15,
            bomb5_center[0] + 15, bomb2_center[1] + 15,
            fill='red')

        # 宝藏
        random.seed(a=8)
        treasure_center = origin + np.array([40 * random.randint(1, WIDTH - 1), 40 * random.randint(1, HEIGHT - 1)])
    ...
    def step(self, action):
        ...
        if s_ == self.canvas.coords(self.treasure):
            ...
            s_ = 'terminal'  # 终止
            print("炸弹 2 爆炸...")
        elif s_ == self.canvas.coords(self.bomb3):
            reward = -1  # 踩到炸弹2，奖励为 -1
            done = True
            s_ = 'terminal'  # 终止
            print("炸弹 3 爆炸...")
        elif s_ == self.canvas.coords(self.bomb4):
            reward = -1  # 踩到炸弹2，奖励为 -1
            done = True
            s_ = 'terminal'  # 终止
            print("炸弹 4 爆炸...")
        elif s_ == self.canvas.coords(self.bomb5):
            reward = -1  # 踩到炸弹2，奖励为 -1
            done = True
            s_ = 'terminal'  # 终止
            print("炸弹 5 爆炸...")
